...
namespace RealDolmenInetum.Components.Pages.Bencher
{
    public partial class Activiteiten
    {
        ...
        // Methode om de benchid van de current user te vinden // HIER MARWAN =============
        ...
        // Methode om de occupation_id van een Bench record te updaten. // MOET AANGEPAST WORDEN
        private async Task<bool> UpdateOccupationInBench(Occupation selectedOccupation)
        {
            var updateResponse = await Http.PutAsJsonAsync($"https://localhost:7256/user/bench/occupation/{benchId}", new { type = selectedOccupationType });
        ...
        private async Task AddOccupationHistory(Occupation selectedOccupation)
        {
            ...
            {
                NavigationManager.NavigateTo($"/activiteit/einde?benchId={benchId}&occupationType={Uri.EscapeDataString(selectedOccupationType)}");
            }
            else
            {
